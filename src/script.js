...
function validateNameAndContact(studentNameEle, studentContactEle) {
	...
	if (
		...
		studentNameEle["value"].length < 4 ||
		!regex.test(studentNameEle["value"])
	) {
		addError(
			studentNameEle,
			"'Must be more than 3 characters and it should all be characters'"
		);
		return false;
	} else {
	...
	if (
		studentContactEle["value"].length < 10 ||
		studentContactEle["value"].length > 10
	) {
...
form.addEventListener("submit", (event) => {
	...
	data = [
		studentNameEle["value"],
		studentIdEle["value"],
		studentEmailEle["value"].trim(),
		studentContactEle["value"],
	];
...
form.addEventListener("keydown", function (event) {
	...
	if (
		event.key === "Enter" &&
		window.getComputedStyle(addBtn).display === "none"
	) {
